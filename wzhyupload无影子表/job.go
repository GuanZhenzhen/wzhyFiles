...
func openSonTable(db ddb.DB, tableName string) ([][]interface{}, error) {
...
func sonRows(rows [][]interface{}, uuid interface{}, id string) ([][]interface{}, error) {
	//查找子表记录内容
	sonRow := [][]interface{}{}
	for _, line := range rows {
		if strings.EqualFold(line[0].(string), id) {
			line[2] = uuid
	...
	for _, maintable := range tables {
		...
		if len(maintable.Details) > 0 {
			for _, sonTable := range maintable.Details {
				sonrow, err := openSonTable(db, sonTable.Name)
				if err != nil {
					return err
				}
		...
		if err := searchTable(maintable.Maxrows, maintable.ID, db, tab, shadowTable, func(i int, rows [][]interface{}) error {
			...
			for _, line := range rows {
				...
				if len(maintable.Details) > 0 {
					for _, sonTable := range maintable.Details {
						sonRow, err := sonRows(sonMap[sonTable.Name], newLine[1], id)
						if err != nil {
							log.Println(err)
							return err
						}
...
func queryDiff(db ddb.DB, table *data.Table, shadowtable string,
	...
	rows, err := db.Query(fmt.Sprintf("select * from %s where %s ", table.FullName(), where), params...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func queryDiff2(db ddb.DB, table *data.Table, shadowtable string, maintable []*detailTab,
	pkvalues []interface{}) ([][]interface{}, error) {
	str := fmt.Sprintf("%s in(?)", table.PrimaryKeys[0])
	where, params, err := data.In(str, pkvalues)
	//查出主表信息
